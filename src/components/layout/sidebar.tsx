...
export function Sidebar({
  ...
  if (isMobile) {
    return <>
        <Button variant="ghost" size="icon" onClick={toggleMobileSidebar} className="fixed left-4 top-3 z-10 xl:hidden">
  ...
  // Desktop sidebar
  return <aside className={cn("group/sidebar relative flex flex-col border-r bg-sidebar transition-all duration-300 ease-in-out", isCollapsed ? "w-[60px]" : "w-[280px]", className)}>
      <div className="flex h-14 items-center border-b px-4">
        <div className={cn("flex flex-1 items-center", isCollapsed && "justify-center")}>
          {!isCollapsed && <div className="overflow-hidden text-ellipsis whitespace-nowrap font-semibold">ERP MJPM</div>}
        </div>
      ...
      <div className="flex items-center justify-center border-t p-2">
        <Button variant="ghost" size={isCollapsed ? "icon" : "sm"} className="w-full justify-start">
