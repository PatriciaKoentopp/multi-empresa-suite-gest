...
export interface ContaPagarRelatorio {
  id: string;
...
export function useRelatorioContasPagar() {
  const { currentCompany } = useCompany();
  const [isLoading, setIsLoading] = useState(false);
  const [contas, setContas] = useState<ContaPagarRelatorio[]>([]);
  ...
  const gerarRelatorio = async (dataReferencia: Date) => {
    ...
    try {
      ...
      // Buscar movimentações de pagar lançadas até a data de referência
      ...
      const parcelasEmAberto: ContaPagarRelatorio[] = [];

      movimentacoes?.forEach((mov) => {
        const favorecidoNome = mov.favorecido?.nome || 'Não informado';
        
        mov.movimentacoes_parcelas?.forEach((parcela) => {
          // Parcela está em aberto se:
          // 1. Nunca foi paga (data_pagamento IS NULL)
          // 2. OU foi paga depois da data de referência
          const foiPagaAteDataRef = parcela.data_pagamento && 
            parcela.data_pagamento <= dataReferenciaStr;
          
          if (!foiPagaAteDataRef) {
