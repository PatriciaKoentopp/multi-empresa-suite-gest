...
export default function FunilConfiguracaoPage() {
  ...
  function handleSalvarEtapa(nova: { nome: string; cor: string; ordem: number }) {
    if (editEtapa) {
      setEtapas(prev =>
        prev.map(e => e.id === editEtapa.id ? { ...e, ...nova } : e)
      );
    } else {
      // Cria nova etapa com id incremental calculado
      const proxId = etapas.length > 0 ? Math.max(...etapas.map(x => x.id)) + 1 : 1;
      setEtapas(prev => [...prev, { id: proxId, ...nova }]);
    }
    setModalOpen(false);
  }
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Etapas do pipeline (Kanban) do CRM</p>
        </div>
      ...
      <FunilFormModal
        ...
        etapa={editEtapa ? { nome: editEtapa.nome, cor: editEtapa.cor, ordem: editEtapa.ordem } : undefined}
      />
    </div>
  );
}
