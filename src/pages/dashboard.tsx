    ...
    const fetchDados = async () => {
      try {
        ...
        const { data: vendas, error: erroVendas } = await supabase
          ...
          .eq('tipo', 'venda')
          .eq('status', 'ativo')
          .gte('data_venda', inicioMesFormatado);
        ...
        if (contasReceber && contasReceber.length > 0) {
          const parcelasAtrasadas = contasReceber.filter(
            parcela => new Date(parcela.data_vencimento) < hoje && parcela.movimentacao?.tipo_operacao === 'receber'
          );
            ...
            parcelasEmAtraso.push(
              ...parcelasAtrasadas.map(parcela => {
                ...
                return {
                  ...
                  status: 'em_aberto' as 'em_aberto', // Tipagem explícita
        ...
        const vendasFormatadas = ultimasVendas?.map(venda => {
          const totalVenda = venda.orcamentos_itens.reduce(
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <SalesDashboardCard
          ...
          description={`${dashboardData.parcelasEmAtraso.length} título(s) em atraso`}
          icon="chart"
        />
        <SalesDashboardCard
        ...
        <SalesDashboardCard
          ...
          description="Novos cadastros no mês"
          icon="users"
        />
      </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {isLoading ? (
                ...
                dashboardData.ultimasVendas.map((venda, index) => (
        ...
        <Card className="col-span-1">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              ) : dashboardData.parcelasEmAtraso.length > 0 ? (
                dashboardData.parcelasEmAtraso.slice(0, 3).map((parcela) => (
                  <div key={parcela.id} className="flex items-start gap-4">
              ...
              {dashboardData.parcelasEmAtraso.length > 3 && (
