...
import { useRelatorioContasPagar } from "@/hooks/useRelatorioContasPagar";
import * as XLSX from "xlsx";

export default function RelatorioContasPagar() {
  const navigate = useNavigate();
  const [dataReferencia, setDataReferencia] = useState<Date>(new Date());
  const { isLoading, contas, resumo, gerarRelatorio } = useRelatorioContasPagar();
  ...
  const exportarExcel = () => {
    ...
    XLSX.writeFile(wb, `contas-a-pagar-${format(dataReferencia, "yyyy-MM-dd")}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Relatório de Contas a Pagar</h1>
          <p className="text-muted-foreground">
            Posição das contas a pagar em aberto em uma data específica
          </p>
        </div>
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">
              Contas a Pagar em Aberto em {format(dataReferencia, "dd/MM/yyyy")}
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center">
              Selecione uma data de referência e clique em "Gerar Relatório" para visualizar as contas a pagar em aberto.
            </p>
          </CardContent>
        </Card>
      )}
