...
import { FileText, Users, Calculator, BarChart, Award, TrendingUp, Clock, Camera, Layers, Cake, CreditCard, Wallet, ScrollText, BookOpen, Receipt } from "lucide-react";
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {relatorios.map(relatorio => {
          ...
          return (
            <Card 
              key={relatorio.id} 
              className={`cursor-pointer transition-shadow hover:shadow-lg ${relatorio.id === 'favorecido' || relatorio.id === 'classificacaoABC' || relatorio.id === 'analiseDRE' || relatorio.id === 'tempo' || relatorio.id === 'fotos' || relatorio.id === 'projetos' || relatorio.id === 'aniversariantes' || relatorio.id === 'financeiro' || relatorio.id === 'contasPagar' || relatorio.id === 'contasReceber' || relatorio.id === 'antecipacoes' || relatorio.id === 'logs' || relatorio.id === 'razaoContabil' ? '' : 'opacity-60'}`}
            ...
            >
              ...
              <CardContent>
                <Button 
                  variant={relatorio.id === 'favorecido' || relatorio.id === 'classificacaoABC' || relatorio.id === 'analiseDRE' || relatorio.id === 'tempo' || relatorio.id === 'fotos' || relatorio.id === 'projetos' || relatorio.id === 'aniversariantes' || relatorio.id === 'financeiro' || relatorio.id === 'contasPagar' || relatorio.id === 'contasReceber' || relatorio.id === 'antecipacoes' || relatorio.id === 'logs' || relatorio.id === 'razaoContabil' ? "default" : "outline"} 
                  className="w-full"
                  onClick={() => handleCardClick(relatorio.route)}
                >
                  Acessar Relatório
