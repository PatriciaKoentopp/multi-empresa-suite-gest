...
BEGIN
  ...
  WHERE o.tipo = 'venda' AND o.status = 'ativo' AND o.data_venda IS NOT NULL AND oi.valor > 0;
  ...
  WITH annual_data AS (
    SELECT 
      ...
      -- Contar apenas orçamentos que têm itens com valor > 0 (mesmo critério do valor total)
      COUNT(DISTINCT CASE WHEN oi.valor > 0 THEN o.id END)::INT AS qtde_vendas,
      ...
      CASE 
        ...
        WHEN ad.year = primeira_venda_ano AND ad.year < ano_atual THEN 
          12 - (SELECT EXTRACT(MONTH FROM MIN(o.data_venda))::INT - 1 
                ...
                AND o.tipo = 'venda' AND o.status = 'ativo' AND oi.valor > 0) + 1
        WHEN ad.year = primeira_venda_ano AND ad.year = ano_atual THEN
